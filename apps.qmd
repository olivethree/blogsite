...
**Data-quality diagnostics for reverse correlation experiments in social psychology.**

[**Package available on GitHub**](https://github.com/olivethree/rcicrdiagnostics) \| [**User's Guide**](https://olivethree.github.io/rcicrdiagnostics/articles/tutorial.html) \| [**Blog post**](posts/p_3/index.qmd) \| [**DOI**](https://doi.org/10.5281/zenodo.19734757)

Reverse correlation pipelines fail quietly. A response column coded `{0, 1}` instead of `{-1, 1}`, stimulus IDs that are off by one, a noise matrix whose dimensions don't match the response data, a parameter file from the wrong stimulus batch — none of these throw errors, but all of them produce a classification image (CI) that looks convincing and is mathematically wrong.

`rcicrdiagnostics` is an R package I put together to catch those silent failures **before** you compute any CI or information value. You give it a response CSV plus your auxiliary stimulus file, call `run_diagnostics()`, and get back a clean pass / warn / fail report per check.

-   **Covers both pipelines** through a single `method` argument: the classical two-image forced choice (2IFC) via the `rcicr` package (Dotsch, 2016, 2023), and Brief-RC ([Schmitz, Rougier, & Yzerbyt, 2024](https://doi.org/10.1002/ejsp.3100)).

-   **Runnable demo ships with the package.** An AI-generated base face (thispersondoesnotexist.com) and bogus-data generators for 2IFC, Brief-RC 12, and Brief-RC 20 let you try the full flow in about a minute without collecting any real study data.

-   **User's guide with a dedicated chapter on CI scaling decisions** — which option to use for infoVal, pixel-wise correlations, rating-task stimuli, and figures. Aims to clear up confusion that shows up in the literature and personal projects.

Citable via Zenodo ([10.5281/zenodo.19734757](https://doi.org/10.5281/zenodo.19734757)). Feedback and bug reports welcome on the [issue tracker](https://github.com/olivethree/rcicrdiagnostics/issues).

------------------------------------------------------------------------

## CARefully prompt your AI

**First steps on eEthical guidance for GenAI-assisted academic writing**

[Launch the CARefully Prompt Library](https://olivethree.github.io/carefullyprompt/)

This interactive prompt library was developed to support PhD students and early-career researchers in navigating the complex ethics of using Generative AI in academic writing activities. This tool was developed for a workshop at the [**2025 SEFI Summer School**](https://sites.google.com/4tu.nl/sefi-phd-summer-school/home)(20-23 May @ Den Bosch, The Netherlands), it operationalizes the recent ethical framework by [**Cheng et al. (2025)**](https://doi.org/10.1186/s41077-025-00350-6) to help writers make informed decisions.

-   **Ethical tiers:** Prompts are classified (Tier 1-3) based on Cheng et al.'s taxonomy, helping you distinguish between "safe" tasks (e.g., editing) and those requiring heavy oversight (e.g., ideation).

-   **Offers a structured starting point:** While not an empirically validated tool (feel free to use it in your educational pilot studies though!), this library offers a structured "sandbox" of templates and examples. Its goal is to inspire better prompting habits and provide a concrete way to reflect on the ethical weight of different writing tasks.
...
[**View Project on GitHub**](https://github.com/olivethree/briefRC) \| [**View Web Demo**](https://olivethree.github.io/briefrc12online/)
...
::: {.callout-note collapse="true" title="Full Stimuli Generation Guide & R Script"}
...
**R script to generate stimuli** *Note: R scripts are adapted from the original materials shared by Schmitz et al. (2024).*
...
::: {.callout-note collapse="true" title="Setup Instructions"}
...
::: {.callout-note collapse="true" title="Data Analysis Code & Reference"}
