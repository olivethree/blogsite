...
-   **Runnable demo ships with the package.** A synthetic base face (thispersondoesnotexist.com) and bogus-data generators for 2IFC, Brief-RC 12, and Brief-RC 20 let you try the full flow in about a minute without collecting any real study data.

-   **User's guide with a dedicated chapter on CI scaling decisions**: which option to use for infoVal, pixel-wise correlations, rating-task stimuli, and figures. Aims to clear up confusion that many researchers out there bump into from time to time.
