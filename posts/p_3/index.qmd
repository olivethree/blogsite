...
-   **`infoval()`** computes per-producer infoVal with a reference distribution keyed on each producer's actual trial count. This closes a calibration gap in the canonical `rcicr::computeInfoVal2IFC()`, which keys the reference on pool size and biases Brief-RC infoVals downward.
